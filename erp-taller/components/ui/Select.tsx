    ...
    return (
      <div className="w-full flex flex-col gap-1.5 relative">
        <div className="relative flex items-center w-full">
          ...
          <select
            ref={ref}
            className={`w-full appearance-none bg-white/70 dark:bg-secondary/70 backdrop-blur-xl border border-white/40 dark:border-white/5 rounded-2xl shadow-sm focus:shadow-soft px-4 py-3 font-body text-sm text-secondary dark:text-neutral-light outline-none transition-all duration-300
              ${Icon ? 'pl-11' : ''} pr-10
          ...
          >
            ...
            {options.map((opt) => (
              <option key={opt.value} value={opt.value} className="bg-white dark:bg-secondary">
                {opt.label}
              </option>
            ))}
          </select>
