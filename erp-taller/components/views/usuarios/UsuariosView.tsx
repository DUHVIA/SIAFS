...
export function UsuariosView({ usuarios, roles }: UsuariosViewProps) {
  ...
  const columns = [
    ...
    { key: 'rol', header: 'Rol', 
      render: (row: any) => (
        <span className="px-2 py-1 rounded-full text-xs font-semibold bg-neutral-light dark:bg-white/10 text-secondary dark:text-neutral-light">
    ...
    { key: 'accesoSistema', header: 'Acceso ERP', 
      render: (row: any) => (
        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${row.accesoSistema ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>
